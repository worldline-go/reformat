...
type SquashOnNonStructType struct {
	InvalidSquashType int `json:",squash"`
}
...
// This funtionallity will be used as we recieve uppercase names from Oracle..
func TestLowerCaseMapNames(t *testing.T) {
	t.Parallel()
...
func TestNormalizeUrlValues(t *testing.T) {
	t.Parallel()

	input := parseForm()

	result := FlattenURLValues(input)
...
// this simulates the results of http.Request's Qeuryparms() function, we use with echo..
func parseForm() url.Values {
	return url.Values{
		"file_id":  []string{"1234", "123"},
		"acquirer": []string{"2"},
	}
}
...
func TestNormalizeURLValues(t *testing.T) {
	...
	result := NormalizeURLValues(input)
...
func TestWeakDBDecode(t *testing.T) {
	t.Parallel()
	...
	var result struct {
		Foo int    `db:"foo_bar"`
		Bar string `db:"bar_foo"`
	}

	if err := WeakDBDecode(input, &result); err != nil {
		t.Fatalf("err: %s", err)
	}
	if result.Foo != 4 {
		t.Fatalf("bad: %#v", result)
